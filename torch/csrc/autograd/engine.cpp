...
auto Engine::backward(const variable_list& variables,
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (!var->creator) {
      ...
      auto it = creator_grad.find(creator);
      if (it == creator_grad.end()) {
          std::unique_ptr<GradBuffer> buf;
        if (creator->requires_grad) {
          buf.reset(new GradBuffer(creator->num_outputs));
          buf->addGrad(var->output_nr, Variable::of(std::move(grad)));
        }
        creator_grad.emplace(creator, std::move(buf));
      // Duplicate creator
      } else {
        if (creator->requires_grad) {
          auto& buf = it->second;
